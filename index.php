...
class Bot
{
    ...
    private $helloUser = "\u{2705} UNDERGROUND DESIGN studio на связи! \u{2705} 
Приветствую, {username}

﹡﹡﹡
Здесь вы можете задать интересующие вас вопросы. \
Пишите, что хотели бы заказать — проконсультирую по срокам выполнения и цене. 

﹡﹡﹡
Канал: @-------------
Jabber/XMPP: --------@xmpp.jp 
(напишите, если нужно зайти) 

----✄--------
Обратите внимание: бот присваивает вашим сообщениям статус «прочитано» автоматически! Это означает только их успешную пересылку. Дождитесь ответа – я отвечу, как только смогу.
(–) Бот не может отобразить редактирование. Поэтому если в сообщении допущена ошибка — просто отправьте новое.
(+) Бот умеет пересылать изображения, гифки и видео.";
    ...
    public function init($data)
    {
        ...
        if($this->isStartBot($arrData)) {
            ...
            // сообщает админу о запуске, выдает юзер, если он есть
            $botstarter = $arrData['message']['from']['username'];
            $this->requestToTelegram(array("text" => "\u{2755} Кто-то запустил бота! Юзер: $botstarter"), $this->adminId, "sendMessage");
        } else {
            // Если это не старт
            if($is_admin)  {
